<div class="bg-white shadow rounded-lg p-6 mb-6">
    <div class="flex justify-between items-center mb-4">
        ...
        <button id="download-medical-records-btn" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded inline-flex items-center">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Function to set the download button's href based on the active tab
    function updateDownloadButton(activeTabTarget) {
        const downloadButton = document.getElementById('download-medical-records-btn');
        if (downloadButton) {
            downloadButton.onclick = function() {
                window.location.href = `{{ route('patient.download') }}?type=${activeTabTarget}`;
            };
        }
    }
